...
Idempotent (MIGR-001): see 0037 — the baseline create_all() may already have
built this table and its indexes on a fresh database.
"""
...
def upgrade():
    bind = op.get_bind()
    if not _has_table(bind, 'offline_devices'):
    ...
    if not _has_index(bind, 'offline_devices', 'ix_offline_devices_business_id'):
        op.create_index('ix_offline_devices_business_id', 'offline_devices', ['business_id'])
    if not _has_index(bind, 'offline_devices', 'ix_offline_devices_user_id'):
